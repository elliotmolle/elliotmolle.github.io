...
def parse_config(path: Path, contract: dict[str, Any]) -> list[SourceConfig]:
    ...
    for index, raw_source in enumerate(source_entries):
        ...
        category = normalize_category(category_value, "Energy", contract)
        if category is None:
...
def normalize_raw_item(
    ...
    *,
    ...
    if topic.casefold() != "energy":
        return None, f"skipped non-public topic {topic!r}"
    ...
    category = normalize_category(category_value, "Energy", contract) or "General"
    if not validate_category("Energy", category, contract):
        category = "General"
    ...
    source_name = normalize_output_value(first_nonempty(raw.get("source"), context.name, "Energy"), DEFAULT_SOURCE_LIMIT)
    if not source_name:
        source_name = "Energy"
...
def collect_source(source: SourceConfig, contract: dict[str, Any], args: argparse.Namespace) -> tuple[FetchOutcome, list[dict[str, Any]]]:
    ...
    if not source.enabled:
        ...
        return outcome, []
    if source.topic.casefold() != "energy":
        outcome.error = f"skipped non-public topic {source.topic!r}"
        return outcome, []
